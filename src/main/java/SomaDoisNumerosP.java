...
public class SomaDoisNumerosP {
    ...
    public static int sumEven(int x) {
        ...
        // então eu paro a recursão e retorno 0
        if (x <= 1) {
            return 0;
        }
        // se x for ímpar, eu ignoro esse x e chamo a função pro número anterior
        if (x % 2 != 0) {
            return sumEven(x - 1);
        }
        // se x for par, eu somo o próprio x com a soma dos pares até x - 2
